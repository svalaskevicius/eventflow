...
object CounterAggregate extends EventFlow[Event, Command] {
  val counting: FlowState[Int] = c => handler(
    when(Increment).emit(Incremented).snapshotAndSwitch(c + 1 -> 'counting),
    when(Decrement).guard(_ => c > 0, "Counter cannot be decremented").emit(Decremented).snapshotAndSwitch(c - 1 -> 'counting)
  )

  val aggregateLogic: Flow[Unit] = handler(
    when[Create].emit[Created].snapshotAndSwitch(evt => evt.start -> 'counting)
  )
